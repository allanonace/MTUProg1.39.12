            ...
            base.AddMethod ( METHODS_SET_MTU_PREFIX + memoryRegister.id,
                new Func<Task<T>> ( async () =>
                {
                         ...
                         memoryRegister.valueType == RegType.BOOL )
                    {
                        ...
                        switch ( Type.GetTypeCode( typeof( T ) ) )
                        {
                            // NOTE: Only boolean values that work at bit level should be written in the MTU in a special way,
                            // NOTE: because the rest of register types do not need anything especial, just a direct copy of the byte array
